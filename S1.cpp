...
overhere:
  std::map<std::string, int> keylist;
    ...
    int i4;                             
    int loss = 0;                           //counter for beacon loss
    std::string x1, x2;
    int x3;
    std::string y1, y2;
    int y3;
...
while ( 1 ) {
        ...
        if (recv(fd, rbuf, sizeof(rbuf)-1, 0) < 0)
            ...
            if(i5 == 1){
                x1 = check_PC_ID(rbuf);
                x2 = get_beacon_id(rbuf);
                x3 = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count();
                
                //Writing the received info on a file
                oo1.open(+"S1 Time per beacon V" + nn, std::ios_base::app);        
                //write on the file
                oo1<<x1 + " " + x2 + " "<< x3 << " 0"<<std::endl;                      
                //close the file
                oo1.close();
